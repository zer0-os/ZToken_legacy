...
describe("zDAOTokenV2 Unit Tests", () => {
  ...
  const testTokenAmount = ethers.utils.parseUnits(DEFAULT_MOCK_TOKEN_AMOUNT, DEFAULT_MOCK_TOKEN_DECIMALS);
  ...
  describe("ZeroDAOTokenV2 Core Functionality", () => {
    ...
    it("should test withdrawERC20 with amount 0 (withdraw all)", async () => {
      ...
      const additionalAmount = ethers.utils.parseUnits(DEFAULT_V2_ADDITIONAL_AMOUNT, DEFAULT_MOCK_TOKEN_DECIMALS);
    ...
    it("should fail when called by non-owner", async () => {
      ...
      const testAmount = ethers.utils.parseUnits(DEFAULT_V2_TEST_AMOUNT, DEFAULT_MOCK_TOKEN_DECIMALS);
      await mockToken.connect(creator).mint(tokenV2.address, testAmount);

      // Try to call withdrawERC20 from non-owner account
      await expect(
        tokenV2.connect(user1).withdrawERC20(
          mockToken.address,
          user1.address,
          testAmount
        )
    ...
    it("should fail when token or recipient address is zero", async () => {
      ...
      await expect(
        tokenV2.connect(creator).withdrawERC20(
    ...
    it("should fail when contract has insufficient token balance", async () => {
      ...
      await expect(
        tokenV2.connect(creator).withdrawERC20(
          mockToken.address,
          user1.address,
          ethers.utils.parseUnits(DEFAULT_V2_INSUFFICIENT_AMOUNT, DEFAULT_MOCK_TOKEN_DECIMALS)
        )
    ...
    it("should allow owner to mint tokens", async () => {
      ...
      expect(await tokenV2.balanceOf(user1.address)).to.eq(initialBalance.add(mintAmount));
    ...
    it("should allow owner to burn tokens", async () => {
      ...
      expect(await tokenV2.balanceOf(user1.address)).to.eq(initialBalance.sub(burnAmount));
    ...
    it("should allow owner to pause and unpause", async () => {
      ...
      await expect(
        tokenV2.connect(user1).transfer(user2.address, ethers.utils.parseEther(DEFAULT_V2_PAUSE_TRANSFER_AMOUNT))
    ...
    it("should handle snapshot functionality", async () => {
      ...
      // Deauthorize user1
      await expect(tokenV2.connect(creator).deauthorizeSnapshotter(user1.address))
        .to.emit(tokenV2, "DeauthorizedSnapshotter")
        .withArgs(user1.address);

      // User1 can no longer snapshot
      await expect(tokenV2.connect(user1).snapshot())
        .to.be.revertedWith("zDAOToken: Not authorized to snapshot");
    });

    it("should handle bulk transfers", async () => {
      const transferAmount = ethers.utils.parseEther(DEFAULT_V2_BULK_TRANSFER_AMOUNT);
      const recipients = [user1.address, user2.address];

      // Ensure creator has enough tokens
      await tokenV2.connect(creator).mint(creator.address, transferAmount.mul(recipients.length));
      ...
      expect(await tokenV2.balanceOf(user1.address)).to.eq(initialUser1Balance.add(transferAmount));
      expect(await tokenV2.balanceOf(user2.address)).to.eq(initialUser2Balance.add(transferAmount));
    });

    it("should handle bulk transferFrom", async () => {
      const transferAmount = ethers.utils.parseEther(DEFAULT_V2_BULK_TRANSFERFROM_AMOUNT);
      const recipients = [user1.address, user2.address];
      ...
      await tokenV2.connect(user1).transferFromBulk(creator.address, recipients, transferAmount);

      expect(await tokenV2.balanceOf(creator.address)).to.eq(initialCreatorBalance.sub(totalAmount));
      expect(await tokenV2.balanceOf(user1.address)).to.eq(initialUser1Balance.add(transferAmount));
      expect(await tokenV2.balanceOf(user2.address)).to.eq(initialUser2Balance.add(transferAmount));
    });
  });
